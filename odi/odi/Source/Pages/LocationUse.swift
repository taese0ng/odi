...
struct LocationUse: View {
    ...
    제1조 (목적)
    이 약관(이하 “약관”이라 합니다)은 ㈜오디(이하 “회사”라 합니다)와 이용 고객(이하 “회원”이라 합니다)간에 회사가 제공하는 서비스(이하 “서비스”라 합니다)를 이용함에 있어 회원과 회사간의 권리, 의무 및 책임사항, 이용조건 및 절차 등 기본적인 사항을 규정함을 목적으로 합니다.
    제2조 (용어의 정의)
    본 약관에서 사용되는 용어의 정의는 다음과 같습니다.
    
    (1) “서비스” : 회사가 제공하는 카페 정보 제공 서비스 및 다양한 온라인 플랫폼 서비스를 의미합니다. 서비스는 구현되는 장치나 단말기(PC, TV, 휴대형 단말기 등의 각종 유무선 장치를 포함하며 이에 한정되지 않음)와 상관 없이 오디 및 오디 관련 웹(Web)∙앱(App) 등의 제반 서비스를 의미하며, 회사가 공개한 API를 이용하여 제3자가 개발 또는 구축한 프로그램이나 서비스를 통하여 이용자에게 제공되는 경우를 포함합니다. 서비스는 다양한 카페 문화가 변하고 기술이 발전함에 따라 지속적으로 함께 변화∙성장해 나갑니다.
    (2) “회원” : 서비스에 접속하여 본 약관에 따라 회사와 이용계약을 체결하고 회사가 제공하는 서비스를 이용하는 고객을 말하며, 회원계정(ID/PW)을 생성하지 않은 일반 고객(일명 비회원 고객)도 포함됩니다.
    (3) “공급자” : 회사가 제공하는 서비스를 이용하여 자신의 카페 정보, 서비스 등을 소개하는 자를 의미하며, 회사로부터 광고 서비스 등을 제공받는 자를 말합니다.
    (4) “게시물” : 회원 및 이용 고객이 서비스에 게시 또는 등록하는 부호(URL 포함), 문자, 음성, 음향, 이미지(사진 포함), 파일 등을 말합니다.
    (5) “쿠폰” : 회원이 서비스를 이용할 때 표시된 금액 또는 비율만큼 이용 금액을 할인 받을 수 있는 할인권을 말합니다. 쿠폰의 종류 및 내용은 회사의 정책에 따라 달라질 수 있습니다.
    ...
    제7조 (회원 정보의 관리)
    ① 회원은 앱 내에 구비된 정보수정 기능 내지 고객센터 등을 통해 개인정보를 열람∙변경∙수정 할 수 있습니다.
    ...
    ④ 회사의 앱 이외에 링크된 사이트에서는 회사의 개인정보처리방침이 적용되지 않습니다. 링크된 사이트 및 서비스를 제공하는 제3자의 개인정보 처리에 대해서는 해당 사이트 및 제3자의 개인정보처리방침을 확인할 책임이 회원에게 있으며, 회사는 이에 대하여 책임을 부담하지 않습니다.
    제3장 서비스 이용
    제10조 (서비스의 이용 개시)
    ...
    제12조 (서비스의 내용)
    ① 서비스는 제2조 제1호에서 정의한 바에 따라 회사가 회원에게 제공하는 카페 관련 온라인 플랫폼 서비스를 말합니다. 서비스는 현재 제공되는 쿠폰등에 한정되지 않으며, 향후 추가로 개발되거나 다른 회사와의 제휴 등을 통해 추가∙변경∙수정될 수 있습니다.
    ...
    ⑤ 서비스 이용에 관한 개별 안내, 쿠폰등에 대한 정보, 쿠폰 사용 시 유의사항 등은 개별 서비스 이용안내∙소개 등을 통해 제공하고 있습니다.
    ...
    ③ 회원이 서비스상에 게재되어 있는 광고를 이용하거나 서비스를 통한 광고주의 쿠폰 등을 이용하는 경우, 이는 전적으로 회원과 광고주 간의 법률관계이므로, 그로 인해 발생한 회원과 광고주간 분쟁 등 제반 문제는 회원과 광고주간에 직접 해결하여야 하며, 이와 관련하여 회사는 어떠한 책임도 지지 않습니다.
    ...
    ③ 회사는 회원이 서비스를 이용하여 기대하는 수익을 상실한 것에 대하여 책임을 지지 않으며 그 밖에 서비스를 통하여 얻은 자료로 인한 손해 등에 대하여도 책임을 지지 않습니다. 회사는 회원이 게재한 게시물(후기 등 포함)의 정확성 등 내용에 대하여는 책임을 지지 않습니다.
    ...
    * 부 칙
    ...
    var body: some View {
        ...
        .navigationBarTitle(Text("위치정보약관"), displayMode: .inline)
        .navigationBarColor(.white)
    }
}

struct LocationUse_Previews: PreviewProvider {
